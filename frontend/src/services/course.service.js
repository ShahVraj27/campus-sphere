...
const courseService = {
  ...
  bulkEnrollInCourse: async (id, userIds) => {
    return api.post(`/courses/${id}/bulk_enroll/`, { user_ids: userIds });
  },
  
  // Bulk unenroll students from a course (admin only)
  bulkUnenrollFromCourse: async (id, userIds) => {
    return api.post(`/courses/${id}/bulk_unenroll/`, { user_ids: userIds });
  },
